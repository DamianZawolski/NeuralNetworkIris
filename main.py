...
print(f"Błąd kwadratowy wyniósł {round(blad_testowy1, 2)}")
...
_, ax = plt.subplots(1)
ax.plot(procenty, label='Dokładność wyrażona w procentach')
fig = plt.gcf()
fig.set_size_inches(10, 8)
ax.set_xlabel('Liczba iteracji')
ax.set_ylabel('Prawidłowo sklasyfikowane kwiaty')
ax.legend()
ax.set_xlim(0, 100)
...
_, ax = plt.subplots(1)
ax.plot(x, lista_bledow, label='Błąd wyrażony w procentach')
fig = plt.gcf()
fig.set_size_inches(10, 8)
...
_, ax = plt.subplots(1)
ax.plot(x, procenty, label='Dokładność wyrażona w procentach w zależności od współczynnika uczenia')
fig = plt.gcf()
fig.set_size_inches(10, 8)
ax.set_xlabel('Współczynnik uczenia')
ax.set_ylabel('Dokładność')
...
ax.set_xlim(0, 2.6)
ax.set_title("Prawidłowo sklasyfikowane kwiaty")
plt.savefig('Współczynniki uczenia- wykres prawidłowych klasyfikacji.png')
plt.show()
